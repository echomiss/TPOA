...
<div id="MainArea"> 
  <!-- 表单内容 -->
  <form action="/index.php/Survey/Index/add" method="post">
    <br />
    <br />
    <!-- 本段标题（分段标题） -->
    <div class="ItemBlock_Title"> 亲爱的同学： <br />
      ...
      <div align="right"> 班级
        <select name="classname" style="width: 150px">
          ...
          <!-- for start -->
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["classname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
          <!-- for end -->
        </select>
        <br />
        <br />
        ...
        <br />
        日期
        <input type="text" name="addtime" value="<?php echo date('Y-m-d H:i:s');?>">
        <br />
      </div>
    </div>
    <br />
